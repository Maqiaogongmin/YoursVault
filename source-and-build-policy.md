...
有思 App 已正式上架 App Store，主 App 源码公开于：

<https://github.com/Maqiaogongmin/yours-app>

当前 YoursVault 仓库继续用于公开发布以下内容：
...
Flutter / iOS / Android 工程源码只进入主 App 仓库。签名文件、本地配置、
开发者个人数据、未清理历史文件和构建产物不得进入任何公开仓库。

## 唯一构建来源

以后所有对外发布的有思安装包，都必须从主 App 公开仓库中已冻结的 commit
构建。发布记录必须写明源码 commit；正式发布应建立不可移动 tag。
...
- 源码没有未使用的 wger 品牌、包名、图标、文案或历史发布残留。
- 必须保留真实的 wger 历史来源、许可证和数据归属说明。
- 源码没有开发者个人训练计划和训练记录。
- 版本号与发布说明一致。
