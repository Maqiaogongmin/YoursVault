...
1. 你自己编排计划，或让安装了 Yours Skills 的 agent 根据目标、时间和器械生成计划。
2. Agent 输出可读 Markdown 和可导入的 `.plan.json`，放入 `Yours Vault/inbox/`。
3. 通过 Yours CLI 验证和试运行，再将计划导入有思。
...
本仓库提供一组给 AI agents / Codex 使用的 Yours Skills：

- `skills/yours-vault`：理解和使用 Yours Vault 开放文件夹协议。
- `skills/yours-cli`：执行导出、检查、验证、试运行和导入。
