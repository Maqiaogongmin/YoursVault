...
YoursVault 是有思的公开发布、开放文件协议和 Agent Skills 仓库，不包含有思 App 的完整源代码。
...
- `Yours-android-arm64-v8a-1.11.1.apk`

较旧 Android 设备可尝试：

- `Yours-android-armeabi-v7a-1.11.1.apk`

模拟器或 x86_64 设备可使用：

- `Yours-android-x86_64-1.11.1.apk`
...
本仓库提供一组给 AI agents / Codex 使用的 Yours Skills。推荐主入口是：

- `skills/yours-agent`：用于分析 Vault、创建新计划、更新现有计划、补充自定义动作、准备 inbox 导入文件和执行可用的校验。

以下 skills 继续保留为兼容入口和细节参考：

- `skills/yours-vault`：Yours Vault 开放文件夹协议参考。
- `skills/yours-plan-author`：计划 JSON 和训练计划写作参考。
- `skills/yours-cli`：CLI 可用时的验证和 dry-run 参考。
